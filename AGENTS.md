...
Recent commits use short, imperative summaries such as `switch to remote SSH execution by default` and `add SENTRY_DSN to backend env template`. Follow that style: one-line, lower-case subject, focused on the change. PRs should include a concise description, affected environments (`dev` or `prod`), any required vault or inventory updates, and relevant command output from `make check`. Include screenshots only if a change affects generated nginx or deployment-facing output that benefits from visual confirmation.
